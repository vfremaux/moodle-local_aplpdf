...
if (!class_exists('local_pdf')) {
    ...
    if (!class_exists('VFTCPDF')) {
        if (!empty($config->enablelocalpdf)) {
            ...
            // knows it may exist.
            require_once($CFG->dirroot.'/local/aplpdf/vftcpdf/vfclassrouter.php');
        } else {
            require_once($CFG->dirroot.'/lib/pdflib.php');
            require_once($CFG->dirroot.'/local/aplpdf/vftcpdf/standardclassrouter.php');
        }
    }
    ...
    function tcpdf_add_standard_plugin_settings(&$settings, $plugin, $defaultdocument = '') {
        ...
        $settings->add(new admin_setting_heading($key, get_string('configpdfgeneration', 'local_aplpdf'), ''));
        ...
        $label = get_string('configpdfenabled', 'local_aplpdf');
        $desc = get_string('configpdfenabled_desc', 'local_aplpdf');
        $default = 0;
        ...
        $label = get_string('configprintconfig', 'local_aplpdf');
        $desc = get_string('configprintconfig_desc', 'local_aplpdf');
        $default = '';
        ...
        $label = get_string('configdefaulttemplate', 'local_aplpdf');
        $desc = get_string('configdefaulttemplate_desc', 'local_aplpdf');
        ...
        $label = get_string('configdocwatermarkimage', 'local_aplpdf');
        $desc = get_string('configdocwatermarkimage_desc', 'local_aplpdf');
        ...
        $label = get_string('configdoclogoimage', 'local_aplpdf');
        $desc = get_string('configdoclogoimage_desc', 'local_aplpdf');
        ...
        $label = get_string('configdocheaderimage', 'local_aplpdf');
        $desc = get_string('configdocheaderimage_desc', 'local_aplpdf');
        ...
        $label = get_string('configdocinnerheaderimage', 'local_aplpdf');
        $desc = get_string('configdocinnerheaderimage_desc', 'local_aplpdf');
        ...
        $label = get_string('configdocfooterimage', 'local_aplpdf');
        $desc = get_string('configdocfooterimage_desc', 'local_aplpdf');
        ...
        $label = get_string('configdocinnerfooterimage', 'local_aplpdf');
        $desc = get_string('configdocinnerfooterimage_desc', 'local_aplpdf');
    ...
    function tcpdf_draw_frame_letter($pdf) {
        ...
        if (($printconfig->bordercolor ?? 0) > 0) {
            if ($printconfig->bordercolor == 1) {
                $color = [0, 0, 0]; // Black.
            }
            if ($printconfig->bordercolor == 2) {
     ...
     */
    function tcpdf_print_image($pdf, $context, $component, $filearea, $itemid = 0,
            $x = 0, $y = 0, $w = 0, $h = 0) {
        global $CFG;
    ...
    /**
     * Utility to print a QRCode in pdf documents pre-choosing QRCode style and options.
     * @param object $pdf
     * @param string $code
     * @param string|moodleurl $targeturl
     * @param int $x
     * @param int $y
     * @return void
     */
